...
const LikeDislikeActions = ({ initialScore, commentId, restaurantId }) => {
    ...
    useEffect(() => {
        const rateComment = async (action) => {
            try {
                await axios.post(`${process.env.REACT_APP_API_URL}/api/restaurant/${restaurantId}/rate-comment/${commentId}`, { action });
            } catch (error) {
                console.error(error);
            }
        };
    ...
    }, [userAction, commentId, restaurantId]);
